...
def main(sourceURL):
  ...
  [c.unwrap() for c in content.findAll("code", {"class": "highlighter-rouge"})]
  [c.decompose() for c in content.findAll(["code", "table"])]
  [c.decompose() for c in content.findAll("div", {"class": "syntaxhighlighter"})]
  [t.decompose() for t in content.findAll("a", {"class": "meta-comments"})] # why not working???
